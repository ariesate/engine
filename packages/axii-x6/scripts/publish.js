...
try {
  ...
  const { version, name } = await loadJSON(path.join(process.cwd(), './package.json'))
  console.log(`git add -A`)
  await exec(`git add .`)
  console.log(`git commit -m "version: ${name} ${version}"`)
  await exec(`git commit -m "version: ${name} ${version}"`)
