...
export default function ConfigPanel({ graph, type, item }) {
  console.log("rerender", item)
  return (
    <div className={styles.config}>
      {() => type.value === 'graph' ? <ConfigGrid graph={graph} /> : null}
      {() => type.value === 'entity' ? <ConfigEntity entity={item} graph={graph} /> : null}
      {() => type.value === 'relation' ? <ConfigRelation relation={item} graph={graph} /> : null}
    </div>
  )
}
